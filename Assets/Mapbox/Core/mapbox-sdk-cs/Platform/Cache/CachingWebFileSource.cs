...
#region idisposable
		...
		)
		{
			...
			tilesetId = tilesetId.ToString();
			//UnityEngine.Debug.Log("tileId is..."+tilesetId);
			Match http = Regex.Match(tilesetId,"^(http|https)://mt1.*$");
			if(http.Success) {
				 mmiurl = tilesetId+_accessToken+"/still_map/"+tileId+".png";
			...
			{
				uri = tilesetId;
			    uri = Regex.Replace(uri,"\\{z\\}",tileId.Z.ToString(),RegexOptions.IgnoreCase);
				uri = Regex.Replace(uri,"\\{x\\}",tileId.X.ToString(),RegexOptions.IgnoreCase);
				uri = Regex.Replace(uri,"\\{y\\}",tileId.Y.ToString(),RegexOptions.IgnoreCase);
				//UnityEngine.Debug.Log("uri value is"+uri);

				
			}*/
			//UnityEngine.Debug.Log("uri val is"+mmiurl);
			var uriBuilder = new UriBuilder(mmiurl);

			if (!http.Success && !string.IsNullOrEmpty(_accessToken))
			{
			...
			//UnityEngine.Debug.Log("finalUrl is ..............."+finalUrl);


#if MAPBOX_DEBUG_CACHE
			string methodName = _className + "." + new System.Diagnostics.StackFrame().GetMethod().Name;
			//UnityEngine.Debug.Log("methodName is..."+methodName);
#endif

			// if tile  examplewas available call callback with it, propagate to all other caches and check if a newer one is available
			if (null != cachedItem)
			{
#if MAPBOX_DEBUG_CACHE
		...
		{
			//UnityEngine.Debug.Log("the new url is........"+url);
