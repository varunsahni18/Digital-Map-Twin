		...
		public IAsyncRequest Get(string tilesetName, Action<TileJSONResponse> callback)
		{
			string url = string.Format(
				...
				, tilesetName
                        ...
                        Match http = Regex.Match(tilesetName,"^(http|https)://.*$");
			if(http.Success)
			{
				url = tilesetName + "/HDUnitymetadata.json";
				//url = tilesetName + "/style.json";
			...
			return _fileSource.Request(
				...
				, (Response response) =>
				{
					//UnityEngine.Debug.Log("response data is......"+response.Data);
