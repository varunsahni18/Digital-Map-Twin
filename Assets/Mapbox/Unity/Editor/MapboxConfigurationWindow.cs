namespace Mapbox.Editor
{
	...
	public class MapboxConfigurationWindow : EditorWindow
	{
		...
		static void OpenWindow()
		{
			...
			instance.titleContent = new GUIContent("MapMyIndia Setup");
			instance.Show();
		...
		{
			//UnityEngine.Debug.Log("the stat "+status);
		...
		void OnGUI()
		{
			...
			{
				EditorGUILayout.BeginVertical(_verticalGroup);
				//UnityEngine.Debug.Log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!???????????????????"+_sampleContent.Length);
				DrawExampleLinks();
				EditorGUILayout.EndVertical();
			}
		...
		void DrawAccessTokenLink()
		{
			...
			else
			{
				...
				GUIContent labelContent = new GUIContent("Add your MMI API Key");
				GUIContent linkContent = new GUIContent("mmiapi/login/api_key");

				EditorGUILayout.LabelField(labelContent, _bodyStyle, GUILayout.Width(_bodyStyle.CalcSize(labelContent).x));

				if (GUILayout.Button(linkContent, _linkStyle))
				{
					Application.OpenURL("https://www.mapmyindia.com/api/login");
				}
		...
		void DrawAccessTokenField()
		{
			...
			else
			{
				...
				{
					//UnityEngine.Debug.Log("the _lastValidatedToken is............. "+_lastValidatedToken);
					//UnityEngine.Debug.Log("the _accessToken is............. "+_accessToken);

					mmiurl = "https://mt1.mapmyindia.com/advancedmaps/v1/"+_accessToken+"/still_map/16/46894/30386"+".png";
      					//UnityEngine.Debug.Log("the mmiurl is.."+mmiurl);

					//validToken
					try {
     						string HTMLSource = wc.DownloadString(mmiurl);
					...
					//invalidToken
						...
						GUI.contentColor = _defaultContentColor;
						GUI.backgroundColor = _defaultBackgroundColor;
						EditorGUI.BeginDisabledGroup(_currentTokenStatus != MapboxTokenStatus.TokenValid || _validating);
		...
		void DrawChangelog()
		{
			...
			if (GUILayout.Button(linkContent, _linkStyle))
			{
				Application.OpenURL("http://dmt.vidteq.com/sdk/unity/index.php");
			}
		...
		{
			try {
				//UnityEngine.Debug.Log("inside try");
     				string HTMLSource = wc.DownloadString(mmiurl);
				EditorGUILayout.LabelField("Map Prefabs", _titleStyle);
				//EditorGUI.BeginDisabledGroup( _validating);
		...
		{
			//UnityEngine.Debug.Log("MMIStatus "+mmiStatus);
			if(mmiStatus) {
				//UnityEngine.Debug.Log("inside try");
			...
			if(!(mmiStatus)) {
				//UnityEngine.Debug.Log("inside exception");
				EditorGUILayout.LabelField("Enter valid MMI_LicKey", _titleStyle);
				EditorGUILayout.LabelField("Example Scenes", _titleStyle);
				EditorGUI.BeginDisabledGroup(_currentTokenStatus != MapboxTokenStatus.TokenValid || _validating);
				EditorGUI.BeginDisabledGroup(_currentTokenStatus != MapboxTokenStatus.TokenValid || _validating);
				//EditorGUI.BeginDisabledGroup((mmiStatus));
